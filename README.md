...
Show Now sends a test notification even when periodic reminders are disabled. Enabled controls the random-interval timer (20–30 minutes by default on macOS). Nonempty lines from **all saved packs** rotate one per pack in pack-name order. Each pack advances through its own lines and wraps independently, so a large pack cannot crowd out smaller packs. The shared next position survives restarts; Show Now also advances the sequence. The pack selector chooses which pack to edit, not which packs receive reminders. Existing packs, intervals, login settings, and saved preferences are preserved. The former Display Duration preference is retained for compatibility but does not control native notifications. Other platforms retain upstream behavior.
